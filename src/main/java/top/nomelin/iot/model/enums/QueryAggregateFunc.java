...
public enum QueryAggregateFunc {
    AVG("平均", "对聚合时间粒度内的数值取平均值。若为字符串类型，则等同于FIRST"),
    MAX("最大值", "对聚合时间粒度内的数值取最大值。若为字符串类型，则取最长的字符串"),
    MIN("最小值", "对聚合时间粒度内的数值取最小值。若为字符串类型，则取最短的字符串"),
    SUM("总和", "对聚合时间粒度内的数值求和。若为字符串类型，则等同于COUNT"),
